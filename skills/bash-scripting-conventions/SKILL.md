---
name: bash-scripting-conventions
description: "Shell script safety rules: `set -euo pipefail`, `trap` cleanup, quoted expansions, `[[ ]]` conditionals, `mktemp` for temp files, shellcheck-clean. Apply when writing or reviewing a shell/bash script, or a prompt mentions a .sh file or shell scripting — triggers on topic, not file path. For scaffolding a new script from scratch see bash-automation-workflow."
---
