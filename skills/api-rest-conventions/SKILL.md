...
description: "REST endpoint design rules: plural-noun resources, correct HTTP verbs, pagination, versioning, idempotency keys, consistent error envelopes. Apply when writing or reviewing an HTTP route/endpoint/handler/controller, or a prompt mentions endpoints, resources, routes, API contracts, or pagination — triggers on topic, not file path. For the end-to-end design process (lock contract -> edge cases -> tests) see api-endpoint-design-workflow."
---
