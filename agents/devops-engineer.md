...
4. **Least privilege** — deployment credentials/IAM roles are scoped to only what the pipeline needs (see the `aws-conventions` skill); prefer short-lived/OIDC auth over long-lived static credentials.
...
2. For shell steps, follow the `bash-scripting-conventions` skill (quoting, `set -euo pipefail`, no unvalidated interpolation into commands).
